...
public class Employee extends People {
		...
		try {
			...
			try {
				...
				if (rs.first()) {
					...
					String retrievedOfficeAddress = rs
							.getString("OfficeAddress");
			...
			try {
				...
				if (isExisting) {
					...
					PreparedStatement stmt = conn
							.prepareStatement(SQLEmployeeInsert);
					stmt.setInt(1, UIN);
	...
	public static boolean addEmployeeCheck(int UIN) {
		...
		try {
			...
			try {
				...
				PreparedStatement stmt = conn
						.prepareStatement(SQLEmployeeSelect);
				stmt.setInt(1, UIN);
				ResultSet rs = stmt.executeQuery();
	...
	public static boolean updateEmpDetails(int UIN, String officeAddress,
			String officeHours) {
		...
		try {
			...
			try {
				...
				if (isExisting) {
					...
					// database....NOT ADD
					System.out
							.println("Updating the emp details in the database");
	...
	public static boolean giveBonus(int UIN, double bonusPercent)
			throws bonusNotValidException {
			...
			try {
				if (conn != null) {
					...
					if (rs.first()) {
						...
						System.out
								.println("UIN does not exist in the employee table");
